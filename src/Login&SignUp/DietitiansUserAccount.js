...
function DietitiansUserAccount() {
    useEffect(() => {
    ...
    const login = async () => {
        ...
        try {
            ...
            if (imageFile) {
                try {
                    const imageRef = ref(storage, `verificationDocs/${imageFile.name}`);
                    await uploadBytes(imageRef, imageFile);
                    imageUrl = await getDownloadURL(imageRef);
                } catch (fileUploadError) {
                    alert('Error uploading file: ' + fileUploadError.message);
                    return;
                }
            }
            ...
            if (emailMatchingArray.length > 0) {
                ...
                const userType = 'DietitiansUser'; // Set userType to PremiumUser directly
                await addDoc(dbref, {
                    Name: name,
                    Email: email,
                    Password: password,
                    ...
                    VerificationDoc: imageUrl, // Save image URL in the user's document
                    status: 'pending',
    ...
    return (
        <div>
            ...
            <div className='login-container'>
                <div className='form'>
                    <h2 className='UserAccount-title'>Sign In As a Dietitians User</h2>
                    ...
                    <div className="form-group">
                        <label>For verification Purpose, Please upload either of the following:</label>
                        <p>Copy of licence Certificate</p>
                        <p>Copy of Undergraduate Transcript/Postgraduate Certificate</p>
...
export default DietitiansUserAccount;
