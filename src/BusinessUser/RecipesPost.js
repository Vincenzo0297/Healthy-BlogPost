...
function RecipesPost() {
  ...
  const handleDelete = async (postId) => {
    try {
      await deleteDoc(doc(db, 'RecipesPost', postId));
      console.log('Recipes Post deleted successfully');
      fetchPosts();
    } catch (error) {
      console.error('Error deleting Recipes Post:', error);
    }
  };

  const handleUpdate = (postId) => {
    navigate(`/UpdateRecipesPost/${postId}`);
  };

  const handleView = (post) => {
    if (post.suspended) {
      alert("This Recipes Post is suspended and cannot be viewed.");
    } else {
      navigate(`/ViewRecipesPost/${post.id}`);
    }
  };
  ...
  const handleSuspend = async (postId, isSuspended) => {
    try {
      ...
      console.error('Error suspending Recipes Post:', error);
    }
  };
  ...
  return (
    <div>
      ...
      <div className='user-account-management'>
            ...
            <table className="posts-table">
              ...
              <tbody>
                {filteredPosts.map((post) => (
                  <tr key={post.id}>
                    <td>{post.title}</td>
                    <td>{post.dietaryPreference}</td>
                    <td>{post.datecreated.toLocaleDateString()}</td>
                    <td>{post.category}</td>
                    <td>
                      <button className="button-button" onClick={() => handleUpdate(post.id)}>Update</button>
                      <button className="button-button" onClick={() => handleDelete(post.id)}>Delete</button>
        ...
        <button className="button-button" onClick={handleCreateClick}>Create</button>
