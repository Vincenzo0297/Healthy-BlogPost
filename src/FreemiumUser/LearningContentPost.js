...
function LearningContentPost() {
  ...
  useEffect(() => {
    ...
    const fetchBlogPosts = async () => {
      try {
        const blogPostsCollection = collection(db, 'posts');
  ...
  if (loading) {
    return <div>Loading Content posts...</div>;
  }
      ...
      <div className="blog-posts-container">
        ...
        <div className="search-filter">
            <div className="toolbar">
                <input
                    type="text"
                    placeholder="Search Content Post..."
                    value={searchTerm}
        ...
        <div className="card-grid">
          {filteredPosts.length === 0 ? (
            <p>No Content Post found.</p>
          ) : (
            filteredPosts.map((post) => (
              <Link to={`/ViewContentPost/${post.id}`} key={post.id} className="card-link">
                <div className="card">
...
export default LearningContentPost;
