...
function RecipesPost2() {
  ...
  useEffect(() => {
    ...
    const fetchBlogPosts = async () => {
      try {
        const blogPostsCollection = collection(db, 'RecipesPost');
  ...
  if (loading) {
    return <div>Loading Recipe posts...</div>;
  }
      ...
      <div className="blog-posts-container">
        ...
        <div className="search-filter">
            <div className="toolbar">
                <input
                    type="text"
                    placeholder="Search Recipe Post..."
                    value={searchTerm}
        ...
        <div className="card-grid">
          {filteredPosts.length === 0 ? (
            <p>No Recipe posts found.</p>
          ) : (
            filteredPosts.map((post) => (
              <Link to={`/ViewRecipesPost/${post.id}`} key={post.id} className="card-link">
                <div className="card">
