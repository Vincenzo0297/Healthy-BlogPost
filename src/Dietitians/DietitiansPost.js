...
function DietitiansPost() {
  ...
  const fetchPosts = async () => {
    try {
      ...
      console.error('Error fetching Dietitians posts:', error);
    }
  };

  const handleCreateClick = () => {
    navigate('/CreateDietitiansPost');
  };
  ...
  const handleDelete = async (postId) => {
    try {
      await deleteDoc(doc(db, 'DietitiansPost', postId));
  ...
  const handleUpdate = (postId) => {
    navigate(`/UpdateDietitiansPost/${postId}`);
  };

  const handleView = (postId) => {
    navigate(`/ViewDietitiansPost/${postId}`);
  };
            ...
            <table className="posts-table">
              ...
              <tbody>
                {filteredPosts.map((post) => (
                  <tr key={post.id}>
                    <td>{post.title}</td>
                    <td>{post.description.length > 50 ? `${post.description.substring(0, 50)}...` : post.description}</td>
